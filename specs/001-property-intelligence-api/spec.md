...
- What happens when a data provider is temporarily unavailable? The system must
  ...
  numeric score. The `providers_unavailable` array lists each failed enabled provider.
...
- **FR-007**: System MUST cache enrichment data per source with TTLs
  ...
  data like crime, longer for stable data like census figures). TTLs MUST be
  resolved from provider registry metadata, not from a hardcoded provider list.
...
- **FR-012**: Trend direction (improving/stable/worsening) MUST be computed
  ...
  production import.
- **FR-013**: System MUST support a declarative provider registry loaded from
  configuration and/or deployment metadata. Each registry entry MUST define at
  least `provider_id`, enabled state, supported capability/dimension, cache TTL,
  timeout, source type, and version metadata.
- **FR-014**: Enabling or disabling a provider MUST be possible without code
  changes to the API contract, scoring engine, or persistence schema.
- **FR-015**: Provider orchestration MUST execute only enabled registry entries.
  Disabled providers MUST be skipped before fetch fan-out and MUST NOT be
...
- **PropertyProfile**: Aggregated enrichment data from all enabled providers for a
...
- **DataProviderResult**: Raw enrichment payload from a single enabled external source,
  tagged with provider ID, fetch timestamp, and TTL.
- **ProviderCatalogEntry**: Declarative metadata for one source adapter,
  including `provider_id`, display name, enabled flag, supported
  capability/dimension, cache TTL, timeout, source type, and version.
...
- Overpass API (OpenStreetMap POIs) and ViaCEP are called in real-time per
  ...
  caching and controlled through the provider registry like any other source.
- The initial MVP registry includes eight providers, but the platform is
  intentionally designed so sources can be added or removed without changing the
  endpoint schema or the six fixed scoring dimensions.
